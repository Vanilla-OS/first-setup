    ...
    def get_setup_commands(log_path, pre_run, post_run, commands):
        ...
        with open(commands_script_path, "w") as f:
            ...
            if len(next_boot) > 0:
                ...
                f.write("echo 'echo \"[Desktop Entry]\" > " + user_fs_desktop_path + "\"' >> " + next_boot_script_path + "\n")
                f.write("echo 'echo \"Name=FirstSetup\" >> " + user_fs_desktop_path + "\"' >> " + next_boot_script_path + "\n")
                f.write("echo 'echo \"Comment=FirstSetup\" >> " + user_fs_desktop_path + "\"' >> " + next_boot_script_path + "\n")
                f.write("echo 'echo \"Exec=vanilla-first-setup\" >> " + user_fs_desktop_path + "\"' >> " + next_boot_script_path + "\n")
                f.write("echo 'echo \"Terminal=false\" >> " + user_fs_desktop_path + "\"' >> " + next_boot_script_path + "\n")
                f.write("echo 'echo \"Type=Application\" >> " + user_fs_desktop_path + "\"' >> " + next_boot_script_path + "\n")
                f.write("echo 'echo \"NoDisplay=true\" >> " + user_fs_desktop_path + "\"' >> " + next_boot_script_path + "\n")

                f.write("echo '[Desktop Entry]' > " + next_boot_autostart_path + "\n")
                f.write("echo 'Name=FirstSetup Next Boot' >> " + next_boot_autostart_path + "\n")
                f.write("echo 'Comment=Run FirstSetup commands at the next boot' >> " + next_boot_autostart_path + "\n")
                f.write("echo 'Exec=vanilla-first-setup --run-post-script \"%s env REAL_USER=$USER %s\"' >> " % (pkexec_bin, next_boot_script_path) + next_boot_autostart_path + "\n")
                f.write("echo 'Terminal=false' >> " + next_boot_autostart_path + "\n")
                f.write("echo 'Type=Application' >> " + next_boot_autostart_path + "\n")
                f.write("echo 'X-GNOME-Autostart-enabled=true' >> " + next_boot_autostart_path + "\n")
            # fmt: on
